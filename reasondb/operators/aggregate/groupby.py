...
from reasondb.query_plan.logical_plan import LogicalPlanStep, LogicalGroupBy
...
from reasondb.reasoning.observation import GroupByObservation, Observation
...
class GroupBy(PhysicalOperator):
    async def get_observation(
        ...
        group_column_identifiers: List[VirtualColumnIdentifier] = llm_parameters[
            "group_columns"
        ]
        group_columns = [
            VirtualColumn(col.name, database_state.get_data_type(col))
            for col in group_column_identifiers
        ]

        assert len(inputs) == 1
        groupby_columns_concrete = [
            database_state.get_concrete_column_from_virtual(c) for c in group_columns
        ]

        return GroupByObservation(
            groupby_columns=groupby_columns_concrete,
        )
    ...
    def get_llm_parameters(self) -> PhysicalOperatorInterface:
        return PhysicalOperatorInterface(
            name="GroupBy",
            explanation="Group rows by a set of columns.",
            parameters=[
                LLMParameter(
                    "group_columns",
                    LLMParameterColumnDtype(
                    ...
                    explanation="List of columns to group by.",
                    optional=False,
                    multiple=True,
    ...
    def get_capabilities(self) -> Sequence["BaseCapability"]:
        return (Capabilities.TRADITIONAL_GROUP_BY,)
