...
import torch
from reasondb.backends.audio_qa import AudioQaBackend
...
class AudioQaFilter(PhysicalOperator):
    def __init__(
        self,
        audio_qa_backend: AudioQaBackend,
        quality: float,
        fake_cost: float,
    ):
        self.audio_qa_backend = audio_qa_backend
        self.batch_size = 5
    ...
    async def get_observation(
        ...
        _, columns, audio_column, _ = self.get_params(llm_parameters)

        assert len(inputs) == 1
        assert audio_column.table_name == inputs[0].table_name
        ...
        output_hidden_cols = await database_state.get_output_hidden_cols(
            ...
            if not self.audio_qa_backend.returns_log_odds
            else DataType.FLOAT,
            database_state=database_state,
            logger=logger / "get-skip-columns",
        )
        if self.audio_qa_backend.returns_log_odds:
    ...
    async def _run_outside_db(
        ...
        question, columns, audio_column, keep_answer = self.get_params(llm_parameters)
        cache_dir = database_state.cache_dir

        logger.info(
            __name__,
            f"Running AudioQaFilter with question: {question} with operator {self.get_operation_identifier()}",
        )
        answers, runtime, cost = await self.audio_qa_backend.run(
            question=question,
            audio_column_virtual=audio_column,
            audio_column_concrete=database_state.get_concrete_column_from_virtual(
                audio_column, avoid_materialization_points=True
            ),
            data=input_data,
            boolean_question=True,
            cache_dir=cache_dir,
            logger=logger / "audio-qa-filter",
        )
    ...
    def get_params(self, llm_parameters):
        ...
        audio_column: VirtualColumnIdentifier = llm_parameters["audio_column"]
        keep_answer = llm_parameters["keep_answer"].lower().strip()

        question = question.strip("?") + "?"
        columns = [audio_column]
        return question, columns, audio_column, keep_answer

    def get_operation_identifier(self) -> str:
        return "AudioQaFilter-" + self.audio_qa_backend.get_operation_identifier()

    def get_llm_parameters(self) -> PhysicalOperatorInterface:
        return PhysicalOperatorInterface(
            name="AudioQaFilter",
            explanation="Ask a binary question for each row in the data (needs to contain an audio file) and filter based on the answer.",
            parameters=[
                LLMParameter(
                    "question",
                    LLMParameterValueDtype(dtype_name="str", dtype_func=str),
                    explanation="Binary (yes/no) question to ask for each audio file. Avoid negation in the question. For instance: 'Does the speaker have a male voice?.'",
                    optional=False,
                    free_form=True,
                ...
                LLMParameter(
                    "audio_column",
                    LLMParameterColumnDtype(DataTypes.AUDIO),
                    explanation="Column of the data that contains the audio files which should be used to answer the question. Should be a fully qualified column name (table_name.column_name). For instance 'recordings.audio_file'.",
                    optional=False,
    ...
    def get_capabilities(self) -> Sequence["BaseCapability"]:
        return (Capabilities.AUDIO_ANALYSIS_FILTER,)
    ...
    def setup(self, database: Database, logger: FileLogger):
        try:
            self.audio_qa_backend.setup(logger)
        except Exception as e:
            logger.warning(
                __name__,
                f"Failed to setup AudioQaFilter backend: {e}. This operator will not be available.",
            )

    async def prepare(self, database: Database, logger: FileLogger):
        audio_cols = database.get_concrete_columns_by_type(DataType.AUDIO)

        for audio_col in audio_cols:
            logger.info(
                __name__,
                f"Computing audio kv cache for {audio_col}.",
            )
            get_audio_sql = (
                f"SELECT {audio_col.column_name} FROM {audio_col.table_name} "
            )
            file_paths = []
            for (audio,) in database.sql(get_audio_sql).fetchall():
                file_paths.append(Path(audio))

            await self.audio_qa_backend.prepare(
                column=audio_col,
    ...
    async def wind_down(self):
        await self.audio_qa_backend.wind_down()
