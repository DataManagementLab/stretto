...
# Optimizations of other systems:
# - Palimpzest:
#   - Logical Optimizations:
#       - Filter Reordering: Reorder filters to reduce costs.
#       - Convert Reordering: Delay Converts after filters to reduce the amount of data that needs converting.
#   - Physical Optimizations:
#       - Model Selection: Choose most appropriate model for the task. (lowest cost with acceptable accuracy)
#       - Code Synthesis: Generate code for some tasks when possible.
#       - Multi-Data Prompt Marshalling: Process in row-centric or column centric manner (one field and many rows or one row and many fields at a time)
#       - Input token reduction: minimize input data to the model -> micro RAG to find key excerpts of texts to feed into an LLM
#   - Proposed:
#       - Output Token reduction: Avoid copies from input text by inserting special tokens and asking the llm to output these tokens to select parts of input text.
#       - Model Cascades: Use multiple models in a pipeline to reduce costs.
#
# - Lotus:
#   - Filter:
#       - Cheap proxy model: Learn thresholds when to use the expensive model.
#       - Uses log-probabilities of True and False output tokens to compute those thresholds.
#       - Two thresholds: one for clearly filtered out and one for clearly included.
#   - Join:
#       - Similarity scores of embeddings as proxy score.
#       - Alternative: First map, then proxy score (embedding similarity)
#   - Groupby:
#       - Avoid generating label names using LLM and instead pick label from members using thresholds on embedding similarity with centroid.
#   - Top-k:
#       - Use semantic similarity scores as proxy for ranking (instead of pairwise LLM comparisons)
# - SEED:
#   - Main ones:
#       - CodeGen: Generate code for some tasks when possible.
#       - ModelGen: Finetune small model from labels of the big model.
#       - CacheReuse: Use values from cache to avoid re-computation.
#   - Smaller ones:
#       - Query Batching: Batch a bunch of LLM queries into one.
#       - Integrate tools provided by the user into the solution.
#


# Kinds of operators:
# Filter:
# - Yes/No/Don't know decision per row --> Binary classification
# - Score per row and two thresholds
# - Noise yes/no --> multiple instantiations of the same operator necessary?
# - Linear or sublinear in number of items
# - Cheap / expensive
# - Scoring function to check if decision is correct? (as in frugalgpt)
#
# Extract / Transform:
# - Has to iterate over all rows
...
#
# Join:
# - Yes / No decision per pair --> Only nested loop
# - Score for each join partner and two thresholds
# - O(|A| * |B|) or O(|A| + |B|)
# - Cheap / expensive
# - Noise yes/no --> multiple instantiations of the same operator necessary?
#
...
# Filter:
# - On a small sample S, execute all physcial operators. For each s \in S, decide whether it is selected or not.
#   --> Not sure if all operators should be executed, or whether we should skip some?
# - Based on the result, estimate the runtime and the accuracy of each operator.
# - Each operator can be a model cascade
# - Operators can be merged (e.g. extract multiple columns in one prompt)
#
